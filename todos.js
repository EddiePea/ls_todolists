...
//Detect unauthorised access to routes
...
app.post("/lists", 
  ...
  catchError(async (req, res) => {
    ...
    } else if (await store.todoListTitleExists(todoListTitle)) {
      req.flash("error", "This list title already exists");
      rerenderNewList();
...
app.post("/lists/:todoListId/destroy", 
  ...
  catchError(async (req, res) => {
    let store = res.locals.store;
    let todoListId = req.params.todoListId;
    let listDeleted = await store.deleteTodoList(+todoListId);
